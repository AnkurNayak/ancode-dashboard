...
import {
  ScrollbarGeometry,
  ScrollbarPosition,
} from '@ancode/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
  selector: '[ancodeScrollbar]',
  exportAs: 'ancodeScrollbar',
})
export class AncodeScrollbarDirective implements OnChanges, OnInit, OnDestroy {
  /* eslint-disable @typescript-eslint/naming-convention */
  static ngAcceptInputType_ancodeScrollbar: BooleanInput;
  /* eslint-enable @typescript-eslint/naming-convention */

  @Input() ancodeScrollbar: boolean = true;
  @Input() ancodeScrollbarOptions!: PerfectScrollbar.Options;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // Enabled
    if ('ancodeScrollbar' in changes) {
      // Interpret empty string as 'true'
      this.ancodeScrollbar = coerceBooleanProperty(
        changes?.['ancodeScrollbar'].currentValue
      );

      // If enabled, init the directive
      if (this.ancodeScrollbar) {
        this._init();
      }
    ...
    // Scrollbar options
    if ('ancodeScrollbarOptions' in changes) {
      // Merge the options
      this._options = merge(
        {},
        this._options,
        changes?.['ancodeScrollbarOptions'].currentValue
      );
