...
export class TasksDetailsComponent implements OnInit, AfterViewInit, OnDestroy
{
    ...
    addTagToTask(tag: Tag): void {
        ...
        const tagsFormControl = this.taskForm.get('tags');
        if (tagsFormControl) {
          tagsFormControl.patchValue(this.task.tags);
        }
    ...
    setTaskPriority(priority: any): void {
        const priorityFormControl = this.taskForm.get('priority');
        if (priorityFormControl) {
          priorityFormControl.setValue(priority);
        }
      }
    /**
