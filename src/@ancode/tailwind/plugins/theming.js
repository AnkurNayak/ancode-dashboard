...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--ancode-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--ancode-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--ancode-${name}-rgb))`;
    };
...
// -----------------------------------------------------------------------------------------------------
// @ ANCODE TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--ancode-${e(key)}`, value], [`--ancode-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`ancode.customProps.background.${colorScheme}`);
            const foreground = theme(`ancode.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--ancode-${e(key)}`, value], [`--ancode-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--ancode-${e(key)}`, value], [`--ancode-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
