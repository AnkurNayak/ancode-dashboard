...
import { labelColorDefs } from 'app/modules/admin/dashboards/mailbox/mailbox.constants';
import { MailboxService } from 'app/modules/admin/dashboards/mailbox/mailbox.service';
import { MailFilter, MailFolder, MailLabel } from 'app/modules/admin/dashboards/mailbox/mailbox.types';
...
export class MailboxSidebarComponent implements OnInit, OnDestroy
{
    ...
    folders!: MailFolder[];
    labels!: MailLabel[];
    menuData: AncodeNavigationItem[] = [];
    private _filtersMenuData: AncodeNavigationItem[] = [];
    private _foldersMenuData: AncodeNavigationItem[] = [];
    private _labelsMenuData: AncodeNavigationItem[] = [];
    private _otherMenuData: AncodeNavigationItem[] = [];
    ...
    private _generateFoldersMenuLinks(): void
    {
        ...
        this.folders.forEach((folder) => {
            ...
            const menuItem: AncodeNavigationItem = {
                id   : folder.id,
                title: folder.title,
                type : 'basic',
                icon : folder.icon,
                link : '/dashboards/mailbox/' + folder.slug
            };
    ...
    private _generateFiltersMenuLinks(): void
    {
        ...
        this.filters.forEach((filter) => {
            ...
            this._filtersMenuData.push({
                ...
                type : 'basic',
                icon : filter.icon,
                link : '/dashboards/mailbox/filter/' + filter.slug
            });
        });
    ...
    private _generateLabelsMenuLinks(): void
    {
        ...
        this.labels.forEach((label) => {
            ...
            const colorKey = label.color as keyof typeof labelColorDefs;   
            this._labelsMenuData.push({
                ...
                classes: {
                    icon: labelColorDefs[colorKey].text
                },
                link   : '/dashboards/mailbox/label/' + label.slug
            });
        });
    ...
    private _generateOtherMenuLinks(): void
    {
        ...
        this._otherMenuData.push({
            ...
            link : '/dashboards/mailbox/settings'
        });
    ...
    private _updateNavigationBadge(folders: MailFolder[]): void
    {
        ...
        const mainNavigationComponent = this._ancodeNavigationService.getComponent<AncodeNavigationComponent>('mainNavigation');

        // If the main navigation component exists...
        if ( mainNavigationComponent )
        {
            const mainNavigation = mainNavigationComponent.navigation;
            const menuItem = this._ancodeNavigationService.getItem('dashboards.mailbox', mainNavigation);
